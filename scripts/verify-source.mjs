...
function isExactVersionSpecifier(value) {
...
for (const prefix of ["backend", "mobile"]) {
  ...
  for (const dependencyGroup of ["dependencies", "devDependencies"]) {
    for (const [name, specifier] of Object.entries(packageJson[dependencyGroup] ?? {})) {
      ...
      if (!lockedVersion) {
        fail(`${prefix}: ${name} is missing from the installed lock entries`);
        continue;
      }

      // npm ci already verifies that semver ranges such as ~57.0.12 are
      // satisfied by the package-lock. Only exact pins should equal the
      // installed version character-for-character.
      if (isExactVersionSpecifier(specifier) && specifier !== lockedVersion) {
        fail(`${prefix}: ${name} is pinned to ${specifier} but ${lockedVersion} is installed in the lockfile`);
      }
    }
  }
  ok(`${prefix} top-level dependency lock entries are present and exact pins match`);
}

for (const prefix of ["backend", "mobile"]) {
  const packageJson = json[`${prefix}/package.json`];
  if (packageJson?.engines?.node !== ">=24.0.0 <25") {
    fail(`${prefix}/package.json must pin Node to >=24.0.0 <25`);
  }
}
if (!/^24\./.test(readText(".nvmrc").trim())) fail(".nvmrc must contain a Node 24 version");
