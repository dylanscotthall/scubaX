...
A valid Node result begins with `v24.`.
...
```bash
ss -ltnp | grep -E ':(3000|5433)\b'
```

No output is ideal before the first setup.

ScubaX does not use host port 5432. It uses 5433 by default so an existing native PostgreSQL service can remain untouched. If 5433 is occupied, identify the process before changing anything:

```bash
ss -ltnp | grep ':5433'
```

You may override the project database port for one command, for example:
...
Do not stop the service on port 5432. ScubaX uses 5433. If 5433 is occupied, either stop only the process using 5433 or choose another port:
