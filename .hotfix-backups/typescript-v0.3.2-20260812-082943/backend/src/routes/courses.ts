...
const router = Router();
const courseStaffRoles = [AppRole.INSTRUCTOR, AppRole.ADMIN, AppRole.OWNER];
...
type PublicCourse = Prisma.CourseGetPayload<{ include: typeof coursePublicInclude }>;
...
router.patch(
  ...
  asyncRoute(async (req, res) => {
    ...
    const isAdmin = req.auth!.roles.some((role) =>
      [AppRole.ADMIN, AppRole.OWNER].includes(role),
    );
